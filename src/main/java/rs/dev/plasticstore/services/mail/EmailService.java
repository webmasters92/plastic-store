...
public class EmailService {
    ...
    @Async
    public void sendMessageAnswerEmail(Mail mail) throws MessagingException {
        MimeMessage message = emailSender.createMimeMessage();
        MimeMessageHelper helper = new MimeMessageHelper(message, MimeMessageHelper.MULTIPART_MODE_MIXED_RELATED, StandardCharsets.UTF_8.name());

        Context context = new Context();
        context.setVariable("answer", mail.getMessage());
        context.setVariable("plastic_store_link", mail.getHome_link());
        String html = templateEngine.process("mail/message_answered_mail", context);
